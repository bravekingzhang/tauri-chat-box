...
<template>
    <v-container style="height: 100%">
        <v-list id="myList" style="height: calc(100vh - 100px);overflow-y: scroll;padding: 0px;margin: 0px;">
            <v-list-item v-for="(item, index) in messageList" :key="index">
                <v-col v-if="item.role !== 'user'" style="display: flex; justify-content: start;">
                ...
                <v-col v-else style="display: flex; justify-content: end;">
                    ...
                    </v-card> <v-icon icon="mdi-account-star"></v-icon>
                </v-col>
            ...
            <v-list-item v-if="messageRely">
                <v-col style="display: flex; justify-content: start;">
                    ...
                    <v-card style="padding-left: 45px; padding-right: 10px;">
                        <template v-slot:text>
                            <Markdown :source="messageRely.text" :plugins="plugins"></Markdown>
                        </template>
                    </v-card>
                </v-col>
            </v-list-item>
        ...
        <v-row style="height: 100px">
            <v-col cols="12" align-self="center">
                <v-text-field v-model="messageInput" :append-icon="messageInput ? 'mdi-send' : ''" variant="filled"
                    clear-icon="mdi-close-circle" clearable label="Please input your prompt here" type="text"
                    hint="Press command+enter send message" persistent-hint @keydown.meta.enter.prevent="sendMessage"
                    @click:append="sendMessage">
                    <template #details>
                        See our <a target="_blank" href="https://github.com/bravekingzhang/tarui-gpt-box">Terms and
                            Service</a> here
                    </template>
                </v-text-field>
            </v-col>
