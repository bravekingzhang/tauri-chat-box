    ...
    <v-container style="height: 100%">
        <v-list id="myList" style="
                ...
                <v-col v-if="item.role !== 'user'" style="display: flex; justify-content: start">
                    ...
                    <v-card>
                        <template v-slot:text>
                            <MarkdownText :text="item.text"></MarkdownText>
                        </template>
                    </v-card>
                </v-col>
            ...
            <v-list-item v-if="messageRely">
                <v-col style="display: flex; justify-content: start">
                    ...
                    <v-card>
                        <template v-slot:text>
                            <MarkdownText :text="messageRely.text"></MarkdownText>
                        </template>
                    </v-card>
                </v-col>
            </v-list-item>
        ...
        <v-row style="height: 100px">
            <v-col cols="12" align-self="center">
                <v-text-field color="primary" v-model="messageInput" :append-icon="messageInput ? 'mdi-send' : ''"
                    variant="outlined" clear-icon="mdi-close-circle" clearable label="Please input your prompt here"
                    type="text" hint="Press command+enter send message" persistent-hint
                    @keydown.meta.enter.prevent="sendMessage" @click:append="sendMessage">
                    <template #details>
                        See our
                        <a target="_blank" href="https://github.com/bravekingzhang/tarui-gpt-box">Terms and Service</a>
                        here
                    </template>
                </v-text-field>
            </v-col>
